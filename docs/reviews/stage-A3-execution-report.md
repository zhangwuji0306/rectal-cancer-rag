...
Previous rejected handoff: ffd79b967af9008ca11ebff7720f37c335225239
Prior A3 remediation commit: c65f1231e28eaeaf410c20d3ab3d89e14145d9f5
Remediation commit: 4585aadb63e104abbda80557d22e6b6420b8d6a1
Remediation: 3
...
Blocking findings addressed: source-level 2xx、self-transition、current fulltext 无验证证据和 zero-attempt 活动状态回归均已修复。
...
## 3. Remediation 3：zero-attempt 状态语义收口

- 保持既有双验证合同：所有进入或保持 `fulltext_ready` 的显式转换都要求 `file_valid=True` 与 `bibliographic_match=True`；self-transition 也受同一门禁约束。
- `derive_document_status` 不再无条件信任当前 `fulltext_ready`。仅 current `fulltext_ready` 在无 attempts 时按 metadata 可用性降为 `metadata_only` 或 `pending`；`metadata_ready`、`oa_resolved`、`fetching`、`retryable_error` 在无 attempts 时保持当前状态。
- 有 attempts 时仍按全部 attempts 综合推导；仅 HTTP 200 且无双验证时不会得到 `fulltext_ready`。
...
|---|---|---|
| `直肠癌文献爬取/scripts/state_machine.py` | 保持 fulltext_ready 的双验证与持久化门禁；current fulltext 无 qualifying evidence 时降级；zero-attempt 活动状态保持原状态 | 不会把尚未尝试获取的 `metadata_ready`/`oa_resolved`/`fetching`/`retryable_error` 错误降为 `metadata_only`；既有 fulltext readiness 防绕过行为保持 |
| `tests/test_a3_state_machine.py` | 保留原有 A3 required tests 和上一轮回归；新增四个活动状态 zero-attempt 保持测试 | 覆盖本轮阻塞回归及 AC01–AC04 所需行为 |
| `docs/reviews/stage-A3-execution-report.md` | 更新 remediation 3、commit 边界、测试退出状态、AC 证据和交接状态 | 报告记录本次 A3-only remediation 的最终状态 |
...
因此，current `fulltext_ready` 在无 attempts 或只有未标注 HTTP 200 attempt 时会保守推导为非全文状态；已持久化 marker 的 attempt 在关闭并重新打开 SQLite 后仍可推导为 `fulltext_ready`。其他活动状态在 zero-attempt 时保持当前状态。A3 只消费调用方的验证结果，不新增 A6 内容验证逻辑。
...
|---|---:|---|
| `$env:PYTHONIOENCODING = 'utf-8'; & .venv\Scripts\python.exe tests\test_a3_state_machine.py` | 0 | PASS；20 tests，20 passed |
| `$env:PYTHONIOENCODING = 'utf-8'; & .venv\Scripts\python.exe tests\test_pipeline.py` | 0 | PASS；3 tests，3 passed |
| `$env:PYTHONIOENCODING = 'utf-8'; & .venv\Scripts\python.exe -m unittest discover -s tests -p 'test_*.py'` | 0 | PASS；全量离线 38 tests，38 passed |
...
- `test_zero_attempts_preserve_activity_state`：`metadata_ready`、`oa_resolved`、`fetching`、`retryable_error` 在 zero-attempt 时保持原状态。
...
| A3-AC04 | PASS | `derive_document_status`、`derive_task_status` 与 `record_fetch_attempt` 均基于同一 PMID 的全部 attempts 综合推导；zero-attempt 活动状态保持、SQLite round-trip 和 mixed-source tests 覆盖当前状态语义、跨连接持久化与 source-level 混合结果。 |
...
| Activity state with zero attempts | collapsed to `metadata_only` | preserves current state | fixed |
| A3专项离线测试 | 19/19 | 20/20 | +1 test |
| 全量离线 discovery | 37/37 | 38/38 | +1 test；全通过 |
